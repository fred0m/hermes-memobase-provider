...
class TestCjkKnownNamesConfig(unittest.TestCase):
    """Deployment-specific proper nouns come from config, not the source tree.

    The public build ships an EMPTY ``_CJK_KNOWN``; a deployment injects its
    own names via ``cjk_known_names`` in $HERMES_HOME/memobase.json.
    """

    def setUp(self):
        from memobase_plugin import hybrid_retriever as hr
        self.hr = hr
        self._saved = list(hr._CJK_KNOWN)
    ...
    def test_loader_reads_config(self):
        with tempfile.TemporaryDirectory() as home:
            with open(os.path.join(home, "memobase.json"), "w", encoding="utf-8") as fh:
                json.dump({"cjk_known_names": ["Alice", "Bob"]}, fh)
            self.assertEqual(self.hr.load_cjk_known_names(home), ["Alice", "Bob"])

    def test_loader_missing_file_is_empty(self):
        with tempfile.TemporaryDirectory() as home:
            self.assertEqual(self.hr.load_cjk_known_names(home), [])

    def test_loader_bad_json_is_empty(self):
        with tempfile.TemporaryDirectory() as home:
            with open(os.path.join(home, "memobase.json"), "w", encoding="utf-8") as fh:
                fh.write("{not json")
            self.assertEqual(self.hr.load_cjk_known_names(home), [])

    def test_loader_wrong_type_is_empty(self):
        with tempfile.TemporaryDirectory() as home:
            with open(os.path.join(home, "memobase.json"), "w", encoding="utf-8") as fh:
                json.dump({"cjk_known_names": "Alice"}, fh)
            self.assertEqual(self.hr.load_cjk_known_names(home), [])

    def test_injection_makes_name_an_entity(self):
        self.hr.set_cjk_known_names([])
        self.assertNotIn("Alice", self.hr.extract_entities("chatting with Alice today"))
        self.hr.set_cjk_known_names(["Alice"])
        self.assertIn("alice", [e.lower() for e in self.hr.extract_entities("chatting with Alice today")])

    def test_set_is_idempotent_and_dedupes(self):
        self.assertEqual(self.hr.set_cjk_known_names(["A", "A", "B"]), ["A", "B"])
        self.assertEqual(self.hr.set_cjk_known_names(["A", "A", "B"]), ["A", "B"])
